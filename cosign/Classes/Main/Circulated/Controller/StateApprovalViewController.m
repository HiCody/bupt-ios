...
@interface StateApprovalViewController ()<UITableViewDataSource,UITableViewDelegate>{
    NSInteger index;
}
...
@implementation StateApprovalViewController
- (void)viewDidLoad {
    [super viewDidLoad];
    ...
    //获取参数数组里的字符串
    NSArray *arr = [self.threeModel.checkers componentsSeparatedByString:@","];
    ...
    NSString *str4 = self.threeModel.starterName;
    NSString *str3 = @"审批结束";
...
//        }
    ...
    [self.btn1 setTitle:[NSString stringWithFormat:@"%li",indexPath.row+1] forState:UIControlStateNormal];
...
//    [cell addSubview:headImage];
    ...
    if(indexPath.row ==0 ||[self.statusArr[indexPath.row]isEqualToString:@"创建审批"]){
    ...
    if([self.nameArr[indexPath.row+1]isEqualToString:@"审批结束"]){
    ...
    if ([self.statusArr[indexPath.row]isEqualToString:@"未通过"]||[self.statusArr[indexPath.row]isEqualToString:@"未审批"]) {
...
//        }
  ...
  if ([self.statusArr[indexPath.row]isEqualToString:@"已通过"]) {
    ...
    if ([self.statusArr[indexPath.row]isEqualToString:@"未通过"]) {
        self.btn1.backgroundColor = [UIColor colorWithRed:243/255.0 green:59/255.0  blue:59/255.0  alpha:1.0];
        self.label.textColor = [UIColor colorWithRed:243/255.0 green:59/255.0  blue:59/255.0  alpha:1.0];
    ...
    if (self.nameArr.count ==3 &&[self.statusArr[1]isEqualToString:@"已通过"]) {
        if (indexPath.row==2) {
            self.btn1.backgroundColor = [UIColor colorWithRed:243/255.0 green:59/255.0  blue:59/255.0  alpha:1.0];
            self.label.textColor = [UIColor colorWithRed:243/255.0 green:59/255.0  blue:59/255.0  alpha:1.0];
