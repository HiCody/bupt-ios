...
@interface THistorysViewController ()<UITableViewDataSource,UITableViewDelegate>

@end

@implementation THistorysViewController
...
- (void)viewDidLoad {
    ...
    self.parameters=@{@"userId":account.userName,
                      ...
                      @"id":str,
                      @"rid":self.aModel.rid};
    self.interfaceString = ReadHistoryDetailInteface;
...
- (void)configHeadView{
    ...
    if (self.state==isShow1) {
        ...
        UILabel * approvalLable = [[UILabel alloc] initWithFrame:CGRectMake(15, CGRectGetMaxY(applicationLable.frame), 80, 30)];
        approvalLable.text = @"审批状态  :";
        //approvalLable.textAlignment=NSTextAlignmentRight;
        approvalLable.textColor = [UIColor grayColor];
        approvalLable.font = [UIFont systemFontOfSize:14];
        [self.headView addSubview:approvalLable];
        
        UILabel *firstLabel = [[UILabel alloc]initWithFrame:CGRectMake(100, CGRectGetMaxY(applicationLable.frame),WinWidth-100, 30)];
        firstLabel .text =self.aModel.hqStatusName;
        firstLabel .font = [UIFont systemFontOfSize:14];
        [self.headView addSubview:firstLabel ];
        

        UILabel *copyLable = [[UILabel alloc] initWithFrame:CGRectMake(15, CGRectGetMaxY(approvalLable.frame), 80, 30)];
        ...
        UILabel *secondLabel = [[UILabel alloc]initWithFrame:CGRectMake(100, CGRectGetMaxY(approvalLable.frame), WinWidth-100, 30)];
        CGFloat padding=0;
        if ([self.aModel.readers isEqualToString:@""]) {
            ...
            for (int i=0; i<readerArr.count; i++) {
                UILabel *lable = [[UILabel alloc] initWithFrame:CGRectMake(100, CGRectGetMaxY(approvalLable.frame)+30*i,  WinWidth-100,  30)];
