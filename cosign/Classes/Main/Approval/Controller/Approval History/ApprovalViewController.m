...
@interface ApprovalViewController ()<UITableViewDataSource,UITableViewDelegate,DCToolBarDelegate,UIDocumentInteractionControllerDelegate,UIAlertViewDelegate>{
...
- (void)configToolBar{
 ...
 //   NSArray *rightArr = [userInfo.operateRoles componentsSeparatedByString:@","];
//    if ([rightArr containsObject:@"4"]) {
//        self.hasHighLevel=YES;
...
- (void)configHeadView{
    ...
    if (self.state==!isShow2) {
        ...
        UILabel *doteLabel = [[UILabel alloc]initWithFrame:CGRectMake(100, CGRectGetMaxY(processLabel.frame)+padding-30,WinWidth-100, 30)];
...
-(void)requestData{
    ...
    self.parameters=@{@"userId":account.userName,
                      ...
                      @"id":str,
                      @"hid":self.aModel.hid};
    self.interfaceString = NeedCheckSignDetailInterface;
...
-(void)requestDataWithIndex:(NSInteger)integer{
    ...
    MBProgressHUD *hud =  [MBProgressHUD showMessage:@"正在提交中..."];
    hud.dimBackground=NO;
...
- (void)CheckSignUpRequset{
    ...
    [NetRequestClass NetRequestPOSTWithRequestURL:kCheckSignUp WithParameter:parameters WithReturnValeuBlock:^(id returnValue) {
