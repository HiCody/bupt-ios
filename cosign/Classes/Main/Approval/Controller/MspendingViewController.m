...
@interface MspendingViewController ()<UIScrollViewDelegate,MspendingDelegate,InquireVCdelegate>{
...
@property (nonatomic,strong)NSArray *secondarry;


@end

@implementation MspendingViewController
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(refreshData:) name:MspendingRefresh object:nil];
...
- (void)setUpSegment{
    ...
    self.segmentedControl.sectionTitles = @[@"待审批的事项",@"审批历史"];
    //中间竖线
    ...
    [self.segmentedControl setIndexChangeBlock:^(NSInteger indseg) {
        ...
        [weakSelf.scrollView scrollRectToVisible:CGRectMake(WinWidth * indseg, 0, WinWidth, 200) animated:YES];
    }];
...
-(void)addpassedTableView{
    ...
    self.historyTableView.isSpendHistory=YES;
...
- (void)JumpToSecondView:(NSIndexPath *)indexPath{
    ...
    if (index ==0) {
        ApprovalViewController *MApprovalVC = [[ApprovalViewController alloc]init];
        MApprovalVC.aModel = self.spendingTableView.dataArry[indexPath.row];
        [self.navigationController  pushViewController:MApprovalVC animated:YES];
    }
    if (index == 1) {
        HistoryViewController *HistoryVC =[[HistoryViewController alloc]init];
        HistoryVC.aModel = self.historyTableView.dataArry[indexPath.row];
...
-(void)requestData{
    ...
    [NetRequestClass NetRequestPOSTWithRequestURL:SpendingInterface WithParameter:parameters WithReturnValeuBlock:^(id returnValue) {
...
-(void)requestData1{
    ...
    [NetRequestClass NetRequestPOSTWithRequestURL:SpendHistoryInterface WithParameter:parameters WithReturnValeuBlock:^(id returnValue) {
        NSDictionary *dict = returnValue[@"items"];
