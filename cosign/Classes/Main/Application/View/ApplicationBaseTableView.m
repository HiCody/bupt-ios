...
#import "ApplicationBaseTableView.h"
...
@implementation ApplicationBaseTableView
...
- (void)loadingNewData{
    if (self.applicationDelegate&&[self.applicationDelegate respondsToSelector:@selector(loadNewData)]) {
        
        [self.applicationDelegate loadNewData];
    }
}

-(void)loadingMoreData{
    if (self.applicationDelegate&&[self.applicationDelegate respondsToSelector:@selector(loadMoreDate)]) {
        
        [self.applicationDelegate loadMoreDate];
    }
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    ApplicationModel *app = self.dataList[indexPath.row];
...
//    if ([app.hqTypeName isEqualToString:@"会议申请"]) {
//        bigLabel.text = @"会";
//    }
//    if ([app.hqTypeName isEqualToString:@"用印申请"]) {
//        bigLabel.text = @"用";
//    }
//    if ([app.hqTypeName isEqualToString:@"请款申请"]) {
//        bigLabel.text = @"请";
//    }
//    if ([app.hqTypeName isEqualToString:@"合同审核"]) {
//        bigLabel.text = @"合";
//    }
//    if ([app.hqTypeName isEqualToString:@"公文审阅"]) {
//        bigLabel.text = @"公";
//    }
//    if ([app.hqTypeName isEqualToString:@"其他事项"]) {
//        bigLabel.text = @"其";
//    }
    ...
    bigLabel.textColor = NAVBAR_SECOND_COLOR;
...
//    //附件图片
//    UIImageView *imageview = [[UIImageView alloc]initWithFrame:CGRectMake(60, 5, 24, 21)];
...
//    }
//    else if(![app.fileUrls isEqualToString:@""]){
//        imageview.hidden =NO;
//    }
    ...
    firstLabel.frame = CGRectMake(headLabel.frame.origin.x, (70-firstSize.height)/2.0+2, firstSize.width, firstSize.height);
    ...
    secondLabel.frame = CGRectMake(headLabel.frame.origin.x, CGRectGetMaxY(bigLabel.frame)-secondSize.height, secondSize.width, secondSize.height);
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (self.applicationDelegate&&[self.applicationDelegate respondsToSelector:@selector(JumpToSecondView:)]) {
        
        [self.applicationDelegate JumpToSecondView:indexPath];
    }
}
