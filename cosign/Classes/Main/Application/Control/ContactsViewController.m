...
@interface ContactsViewController ()<UITableViewDelegate,UIScrollViewDelegate,ContactsSelectedPanelDelegate>{
...
@property(nonatomic,strong)ApproverTableView *approverTableView;
...
- (void)viewDidLoad {
    ...
    [self addcontactsTableView];
    [self addapproverTableView];
...
- (void)setUpSegment{
    ...
    self.segmentedControl.sectionTitles = @[@"审批人列表",@"最近审批人列表"];
    //中间竖线
...
-(void)trueExpression:(UIButton *)button{
    if (index ==0){
        ...
        [self.ContactsDelegate changeTitle:str andUserId:str1];
        
        if (self.ContactsDelegate &&[self.ContactsDelegate respondsToSelector:@selector(contactsPassData:)]) {
            [self.ContactsDelegate contactsPassData:self.contactsTableView.selectedArr];
        }
    ...
    if (index ==1) {
        ...
        if (self.approverTableView.index !=-1) {
            NewApplicationModel *appModel =self.approverTableView.approverArr[self.approverTableView.index];
            NSString *str =appModel.checkerDetails;
            ...
            for (NSString *str in tempArr) {
                ...
                [newArray2 addObject:[tempArr1 firstObject]];
            }
            NSString *str1=[newArray componentsJoinedByString:@","];
            NSString *str2=[newArray2 componentsJoinedByString:@","];
            [self.ContactsDelegate  changeTitle:str1 andUserId:str2];
...
- (void)willDeleteRowWithItem:(Userinfos *)user withMultiSelectedPanel:(ContactsSelectedPanel *)multiSelectedPanel{
    //在此做对数组元素的删除工作
//    NSUInteger index1 = [self.contactsTableView.selectedArr indexOfObject:user];
...
//最近会签人列表
-(void) addapproverTableView{
    self.approverTableView= [[ApproverTableView alloc] initWithFrame:  CGRectMake(self.scrollView.frame.size.width,0, self.scrollView.frame.size.width, self.scrollView.frame.size.height)] ;
    
    [self.scrollView addSubview: self.approverTableView];
    
    self.approverTableView.tableFooterView = [[UIView alloc]initWithFrame:CGRectZero];
    
    //    self.approverTableView.mj_header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
    //        [self loadNewData];
    //    }];
    //
    //    // 设置回调（一旦进入刷新状态，就调用target的action，也就是调用self的loadLastData方法）
    //    self.approverTableView.mj_footer = [MJRefreshAutoNormalFooter footerWithRefreshingTarget:self refreshingAction:@selector(loadMoreDate)];
}
...
- (void)loadMoreDate{
    if (index==0) {
        ...
        // 拿到当前的上拉刷新控件，变为没有更多数据的状态
        [self.approverTableView.mj_footer endRefreshingWithNoMoreData];
...
-(void)requestData1{
    ...
    [NetRequestClass NetRequestPOSTWithRequestURL:RecentContactsInterface WithParameter:parameters WithReturnValeuBlock:^(id returnValue) {
        NSDictionary *dict = returnValue[@"items"];
        ...
        if ([self.approverTableView.mj_header isRefreshing]) {
            [self.approverTableView.approverArr removeAllObjects];
            
        }
        
        [self.approverTableView.approverArr addObjectsFromArray:temp];
        
        [self.approverTableView reloadData];
        //[self.approverTableView.mj_header endRefreshing];
        //[self.approverTableView.mj_footer endRefreshing];
        //判断有无数据的时候的视图
        if (self.approverTableView.approverArr.count == 0) {
